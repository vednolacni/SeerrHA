...
Copy-paste ready automations, scripts and dashboard cards for driving Seerr
(Overseerr / Jellyseerr) from Home Assistant.
...
| **[Media available notification](automations/seerr_available_notification.yaml)** | Notifies with poster art once a request flips to `available` and is actually playable. | State (`event_type == 'available'`) |
...
with UI inputs for the notify service, button labels and optional extras.
...
|:---|:---|:---|
| **[Decide request by ID](scripts/seerr_decide_request.yaml)** | Approves or declines a request by its numeric ID and reports the HTTP status back as a persistent notification. | Action call / dashboard button / voice |

---

## Dashboard Cards

| Layout | Description | Card type |
|:---|:---|:---|
| **[Pending requests card](dashboards/pending_requests_card.yaml)** | Poster of the last Seerr event, a markdown summary of who requested what, and Approve/Decline buttons. | `vertical-stack` (native HA cards) |
...
2. Paste a file from [`dashboards/`](dashboards/).
